...
const InputForm: React.FC<InputFormProps> = ({ onCalculate }) => {
    ...
    return (
        ...
        <motion.div 
            ...
            className="relative z-10 w-full max-w-md p-8 bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 shadow-xl"
        >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                    <label className="text-sm font-medium text-purple-200 ml-1">Your Name</label>
                    <input
                        type={hideNames ? "password" : "text"}
                        value={name1}
                        onChange={(e) => setName1(e.target.value)}
                        placeholder="Enter name..."
                        className="w-full px-4 py-3 bg-black/20 border border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 text-white placeholder-white/30 transition-all font-medium"
                    />
                </div>

                <div className="space-y-2">
                    <label className="text-sm font-medium text-pink-200 ml-1">Partner's Name</label>
                    <input
                        type={hideNames ? "password" : "text"}
                        value={name2}
                        onChange={(e) => setName2(e.target.value)}
                        placeholder="Enter partner's name..."
                        className="w-full px-4 py-3 bg-black/20 border border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-500 text-white placeholder-white/30 transition-all font-medium"
                    />
                </div>
